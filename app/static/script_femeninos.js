...
// let genders = {};             // si quisieras usarlo luego
...
function autoDesc({ name, price }) {
  ...
  if (n.includes("rose") || n.includes("rosa")) base.unshift("Notas de rosa fresca");
  if (n.includes("vanilla") || n.includes("vainilla")) base.unshift("Toques dulces de vainilla");
...
function pintarCarrito() {
  ...
  carrito.forEach((it) => {
    ...
    // encabezado con color femenino (clase viene del template)
    clone.querySelector(".rounded-pill").textContent = it.cantidad;
    clone.querySelector(".text-white .lead").textContent = (it.titulo || it.id).toUpperCase();
...
document.addEventListener("click", async (e) => {
  ...
  if (e.target.matches(".card button[data-slug]")) {
    ...
    const ok = (await applyStock(slug, +1)).ok;
    if (!ok) return;
  ...
  if (e.target.matches(".list-group-item .btn-success")) {
    ...
    const ok = (await applyStock(id, +1)).ok;
    if (!ok) return;
  ...
  if (e.target.matches(".list-group-item .btn-danger")) {
    ...
    if (decreased) await applyStock(id, -1);
    pintarCarrito();
  }
